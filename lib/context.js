 ...
 *
 *    this.catchStream(stream[, options]);
 *
...
proto.catchStream = function(stream, options) {
  var ctx = this;
  if (stream.__catchStreamListener) stream.__catchStreamListener();

  var catchStreamListener = stream.__catchStreamListener = function(err) {
    stream.removeListener('error', catchStreamListener);
    stream.__catchStreamListener = null;
    if (err == null) return;
    destroy(stream);
    ctx.onerror(err);
  };
  stream.on('error', catchStreamListener);
  return stream;
};
