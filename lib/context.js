...
const CLEAN_HANDLE = Symbol('Stream#cleanHandle')
...
proto.catchStream = function (stream) {
  if (stream[CLEAN_HANDLE]) throw new Error('"catchStream" has been applied on the stream')

  let streamCleanHandle = (err) => {
    stream.removeListener('error', streamCleanHandle)
    this.removeListener('finish', streamCleanHandle)
    this.removeListener('close', streamCleanHandle)
    stream[CLEAN_HANDLE] = null
  ...
  stream[CLEAN_HANDLE] = streamCleanHandle
  ...
  stream.on('error', streamCleanHandle)
  this.on('finish', streamCleanHandle)
  this.on('close', streamCleanHandle)
  return stream
}
 ...
 * @api private
 */
proto.getStreamCleanHandle = function (stream) {
  return stream && stream[CLEAN_HANDLE] || null
}
