...
function respond () {
  ...
  if (res.headersSent || this._finished) return
...
function onResError (err) {
  ...
  // handler and log.
  if (this.headerSent || this._finished) {
...
function onFinished (ctx, listener) {
  // socket error should be handled by server's 'clientError' listener then destroy
  ctx.res.on('finish', listener)
  // no 'close' event on res
  ctx.req.on('close', function () {
    listener(new Error('socket was closed!'))
  })
}
