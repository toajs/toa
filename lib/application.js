...
Toa.prototype.use = function (fn) {
  assertFn(fn)
  var middleware = fn
  if (!middleware.length && !thunks.isAsyncFn(middleware) && !thunks.isGeneratorFn(middleware)) {
    middleware = function (done) { this.thunk(fn.call(this))(done) }
  }
...
Toa.prototype.toListener = function () {
  ...
  if (mainHandle) {
    middleware.push(function (done) { this.thunk(mainHandle.call(this))(done) })
  }
...
function assertFn (fn) {
  if (!isFn(fn)) throw new Error('require a thunk function or a generator function')
}
