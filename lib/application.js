...
function Toa (server, mainHandle, options) {
  ...
  this.context = Object.create(contextProto, {
    ...
    after: {
      enumerable: true,
      value: function (hook) {
        return this[AFTER_HOOKS].length && this[AFTER_HOOKS].push(toThunkableFn(hook))
      }
    },
    ended: {
      enumerable: true,
  ...
  if (isFn(options)) this.errorHandle = options
  else this.errorHandle = isFn(options.onerror) ? options.onerror : null
...
Toa.prototype.toListener = function () {
  ...
  return function requestListener (req, res) {
    ...
    res.statusCode = 404
    ctx.thunk = thunks(new Scope(ctx, errorHandle))
